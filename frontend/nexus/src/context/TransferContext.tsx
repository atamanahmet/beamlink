    ...
    <TransferContext.Provider
      value={{
        getAllTransfers: async () =>
          (await apiClient.get("/admin/transfers")).data,

        getTransferLogs: async () => (await apiClient.get("/admin/logs")).data,

        getRecentTransferLogs: async (limit = 50) =>
          (await apiClient.get(`/admin/logs/recent?limit=${limit}`)).data,

        getChildTransfers: async (id, type) => {
          if (type === "SINGLE") return [];
          return (
            await apiClient.get(
              `/admin/transfers/${typePath(type)}/${id}/files`,
            )
          ).data;
        },

        initiateDispatch: async (req) =>
          (await apiClient.post("/admin/transfers/send", req)).data,

        pauseTransfer: async (id, type) => {
          await apiClient.post(
            `/admin/transfers/${typePath(type)}/${id}/pause`,
          );
        },

        resumeTransfer: async (id, type) => {
          await apiClient.post(
            `/admin/transfers/${typePath(type)}/${id}/resume`,
          );
        },

        cancelTransfer: async (id, type) => {
          await apiClient.delete(`/admin/transfers/${typePath(type)}/${id}`);
        },

        deleteTransfer: async (id, type) => {
          await apiClient.delete(
            `/admin/transfers/${typePath(type)}/${id}/delete`,
          );
        },
      }}
    >
